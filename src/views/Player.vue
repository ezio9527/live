  ...
  <div class="player">
    ...
    <div class="page-con player-open">
      <div class="header-top">
        <div class="go-back" @click="$router.push({name: 'Home'})">
      ...
      <div class="live-detailBox">
        <div class="my-video">
          <div v-show="playType===1" ref="videoWrap" class="video-wrap"></div>
          <div v-if="playType===2">
        ...
        <div class="live-con">
          <div class="live-box">
            <div class="live-path-kind">
              <div class="go-back-click" @click="$router.push({name: 'Home'})">
                ...
                <div class="popover live-select-item live-popover">
                  <div aria-owns="popover-live-select-item live-popover" class="popover__face">
                    <BaseSelect
                      noneText="暂无动画直播"
                      text="动画直播"
                      :options="details.animationUrl"
                      :active.sync="animationActive"
                      @select="selectAnimationSource"
                    ></BaseSelect>
                  </div>
                </div>
                <div class="popover live-select-item live-popover">
                  <div aria-owns="popover-live-select-item live-popover" class="popover__face">
                    <BaseSelect
                      noneText="暂无視頻直播"
                      text="視頻直播"
                      :options="details.videoUrl"
                      :active.sync="videoActive"
                      @select="selectVideoSource"
                    ></BaseSelect>
                  </div>
                </div>
          ...
          <div class="detail-team-group">
            <div class="detail-team-top">
              ...
              <div class="listop-box">
                <div class="team-group">
                  <div
                    class="sport-icon"
                    :class="{'sport-icon-1': details.type==1, 'sport-icon-2': details.type==2}"
                  ></div>
                  <h1 class="detail-team-group-name">{{details.name}}</h1>
                  <span class="when-time">{{details.matchtime}}</span>
                </div>
                <p class="time">
                  <span></span>
                  <!---->
            ...
            <div class="detail-team-ifo-box">
              <div class="team-ifo">
                <div class="logo" :class="{logo_1: details.type==1,logo_2: details.type==2}">
                  <!---->
                </div>
                <h1 class>{{details.ateam_name}}</h1>
                <!---->
              </div>
              <div class="team-score" v-if="isSocket && msgContent && msgContent.length">
                <span>{{msgContent[0].score[2][0]}}</span>
                -
                <span>{{msgContent[0].score[3][0]}}</span>
              </div>
              ...
              <div class="team-ifo right-ifo">
                ...
                <h1 class>{{details.hteam_name}}</h1>
                <!---->
              </div>
            </div>
          ...
          <!---->
          <!---->
          <p class="text-notice" style="display: none;">
            <span>
              以下数据/信息由程序采集第三方整合得出，仅供参考；
              <br />
            </span>
            <span>本站不保证此数据/信息的准确性、可靠性等任何问题，亦不对使用此数据/信息造成的任何后果负责 。</span>
          </p>
          <!---->
          <!---->
        </div>
...
export default {
  ...
  provide () {
    return {
      globalMsg: this.msgContent,
      getGlobalMsg: () => this.msgContent
    }
  },
  mounted () {
    // 设置语言
