<template>
  <div class="home2">
    <div class="headbar-desktop-default">
      <div class="inner">
        <div class="top-logo router-link-active">
          <img src="@img/logo-white-cn.png" />
        </div>
        <div class="menu-list">
          <a href="/" class="menu-item active">
            <i class="icon icon-zhibo"></i>
            <span class="title">说球帝直播</span>
          </a>
        </div>
        <div class="domains web-text">
          <span>备用域名</span>
          <span>shuoqiudi.live</span>
        </div>
      </div>
    </div>
    <div class="page-con">
        ...
        <div class="header-box">
          <div class="header-top">
            ...
            <div class="navList">
              <a
                href="#"
                :class="{'router-link-exact-active': type===0, 'router-link-active': type===0, 'head_sel': type===0}"
                @click="filtrateCategory(0)"
              >全部</a>
              <a
                href="#"
                :class="{'router-link-exact-active': type===1, 'router-link-active': type===1, 'head_sel': type===1}"
                @click="filtrateCategory(1)"
              >足球</a>
              <a
                href="#"
                :class="{'router-link-exact-active': type===2, 'router-link-active': type===2, 'head_sel': type===2}"
                @click="filtrateCategory(2)"
              >篮球</a>
            </div>
          </div>
          <div class="head-nav __vuescroll">
            <div
              class="__panel __hidebar"
            ...
            >
              ...
              >
                <ul class="horizontal-scroll-list-wrap">
                  <li :class="{'head_sel': categoryId===0}">
                    <a href="/" aria-current="page" @click="filtrateMatch(0)">所有</a>
                  </li>
                  <li
                    :class="{'head_sel': categoryId===item.id}"
                    :data-id="item.id"
                    v-for="(item, index) in categoryFilterList"
                    :key="index"
                    @click="filtrateMatch(item.id, item.type)"
                  >
                    <a href="#">{{item.name}}</a>
                  </li>
                </ul>
      ...
      <div class="scroll-list-wrap-live">
        <div
          class="__vuescroll hBarVisible"
          style="height: 100%; width: 100%; padding: 0px; position: relative; overflow: hidden;"
        >
          <div
            class="__panel __hidebar"
            ref="scrollWrap"
            style="position: relative; box-sizing: border-box; height: 100%; overflow: scroll;overflow-scrolling: touch;-webkit-overflow-scrolling: touch;"
            @scroll="scrollHandle"
            @mousewheel="scrollHandle"
            @DOMMouseScroll="scrollHandle"
          >
            <div
              class="__view"
              style="position: relative; box-sizing: border-box; min-width: 100%; min-height: 100%;"
            ...
            >
              <div class="list-content-scroll">
                ...
                <div class="list-error" v-if="matchList.length === 0 && load">
                  <div class="error">
                    <div class="error-icon">
                      <i class="cubeic-lock"></i>
                    </div>
                    <div class="error-title">数据加载中</div>
                    <!---->
                  </div>
                </div>
                <div class="list-error" v-else-if="matchList.length === 0">
                  <div class="error">
                    <div class="error-icon">
                      <i class="cubeic-warn"></i>
                    </div>
                    <div class="error-title">暂无相关赛事</div>
                    <!---->
                  </div>
                </div>
                <!-- 列表 -->
                <div>
                  <template v-for="(item, index) in matchFilterList">
                    <div class="tab-panel-li" v-if="typeof(item) === 'string'" :key="index">
                      <div class="live-data">{{item}}</div>
                    </div>
                      ...
                      <div class="live-index">
                        ...
                        <div class="listop-box">
                          <div class="team-group">
                            <div
                              class="sport-icon"
                              :class="{'sport-icon-1': parseInt(item.type)===1, 'sport-icon-2': parseInt(item.type)===2}"
                            ></div>
                            <h1 class="level-54 level-99">{{item.name}}</h1>
                          </div>
                          <p class="time changed">
                            <b>{{item.matchTime}}</b>
                            <span>
                              {{convert(item.status)}}
                              <i></i>
                        ...
                        <div class="teamIfo-box">
                          ...
                          <div class="team-score">
                            <span class>{{item.score.split('-')[0]}}</span> -
                            <span class="red">{{item.score.split('-')[1]}}</span>
                          </div>
                          <div class="team-ifo">
                          ...
                          <div class="live-path-box">
                            <div
                              class="live-item pathColor_1"
                              :class="{'pathColor_1': i.status === 1, 'pathColor_3': i.status === 0}"
                              v-for="(i, k) in item.live_urls"
                              :key="k"
                            >
                              <a @click="player(item, 1, k)" class>
                            ...
                            <div
                              class="live-item pathColor_1"
                              v-if="item.live_cartoon_url.length > 0"
                            >
                              <a @click="player(item, 2)" class>
                                <i class="iconfont icon-naozhong"></i>
                                <span>动画直播</span>
                              </a>
                            </div>
      ...
      <div class="refresh" :class="{'refresh-animation': load}" @click="refresh">
      ...
      <div class="footbar no-login-footbar">
        <div class="footbar-inner">
          <div class="menu-list">
            <a href="/" class="menu-item router-link-active active">
              <i class="icon icon-zhibo"></i>
              <!---->
              <span class="title">说球帝直播</span>
            </a>
            <a href="/mine" class="menu-item">
              <i class="icon cubeic-person"></i>
              <!---->
...
export default {
  ...
  methods: {
    ...
    player (item, playType, channel = 0) {
      this.$router.push({ name: 'Details', params: { type: item.type, playType, channel, id: item.id } })
